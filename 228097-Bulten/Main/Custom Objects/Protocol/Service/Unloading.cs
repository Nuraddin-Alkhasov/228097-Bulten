...
namespace HMI.CO.Protocol
{
    class Unloading
    {
        ...
        private void WriteEndToOrders(Run r)
        {
            ...
            ApplicationService.SetVariableValue("", T.Rows[0]["Data_1"].ToString());
            if ((bool)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.04 Tray handling.08 PO.01 Traction.DB PO Traction State.In Position.US 1"))
            {
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.04 Tray handling.09 US 1.01 Belt.DB US 1 Belt HMI.Actual.Order#STRING20", Convert.ToInt32(T.Rows[0]["Data_1"].ToString()));
            }
            else
            {
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.04 Tray handling.10 US 2.01 Belt.DB US 2 Belt HMI.Actual.Order#STRING20", Convert.ToInt32(T.Rows[0]["Data_1"].ToString()));
            }
